...
along with Christoffel.  If not, see <https://www.gnu.org/licenses/>.

"""

from numpy import (absolute, arccos, argsort, around, array, cos, cross, dot,
                   empty, identity, outer, pi, sign, sin, sqrt, tensordot,
                   transpose, zeros)
...
class Christoffel:
    ...
    def __init__(self, stiffness, density):
        ...
        self.stiffness = array(de_voigt(stiffness))
    ...
    def rotate_tensor(self, rot_mat=None, x_dir=None, z_dir=None):
        ...
        if rot_mat is None:
            ...
            if x_dir is not None and z_dir is not None:
                ...
                x_dir -= dot(x_dir, z_dir) * z_dir  # Gram-Schmidt
                x_dir = x_dir / norm(x_dir)
                y_dir = cross(z_dir, x_dir)

                rot = array([x_dir, y_dir, z_dir])
        else:
            rot = rot_mat
        for i in range(4):
            self.stiffness = tensordot(rot, self.stiffness, (1, i))
    ...
    def set_direction_cartesian(self, direction):
        ...
        if z >= 1.0 or z <= -1.0:
            ...
            self.phi = arccos(cos_phi)
            if y < 0.0:
                self.phi = 2.0 * pi - self.phi

        self.christoffel = dot(q, dot(q, self.stiffness))
    ...
    def set_direction_random(self):
        ...
        q = array([cos_phi * sin_theta, sin_phi * sin_theta, cos_theta])
        self.direction = q
        self.phi = phi
        self.theta = arccos(cos_theta)
    ...
    def get_isotropic(self):
        ...
        """
        return array([self.iso_S, self.iso_S, self.iso_P])
    ...
    def set_phase_velocity(self):
        ...
        """
        eig_val, eig_vec = eigh(self.christoffel)
        args = argsort(eig_val)
        eig_val = eig_val[args]
        eig_vec = eig_vec.T[args]

        self._eigenval = eig_val
        self._eigenvec = eig_vec
        self._phase_velocity = sign(eig_val) * sqrt(absolute(eig_val))
    ...
    def set_grad_mat(self):
        ...
        q = self.direction
        tens = self.stiffness
        gradmat = dot(q, tens + transpose(tens, (0, 2, 1, 3)))
        gradmat = transpose(gradmat, (1, 0, 2))
    ...
    def set_group_velocity(self):
        ...
        grad_eig = empty((3, 3))
        group_vel = empty((3, 3))
        self._group_abs = empty(3)
        self._group_dir = empty((3, 3))
        self.group_theta = empty(3)
        self.group_phi = empty(3)
        for pol in range(3):
            for cart in range(3):
                grad_eig[pol][cart] = dot(eig_vec[pol], dot(gradmat[cart], eig_vec[pol]))
            ...
            if z >= 1.0 - 1e-10 or z <= -1.0 + 1e-10:
                ...
                self.group_theta[pol] = arccos(z)
                sin_theta = sqrt(1 - z ** 2)
                if abs(x) > sin_theta:
                    self.group_phi[pol] = (1.0 - sign(x)) * 0.5 * pi
                else:
                    self.group_phi[pol] = arccos(x / sin_theta)
                if self._group_dir[pol][1] < 0.0:
                    self.group_phi[pol] = 2 * pi - self.group_phi[pol]
        # In case things go wrong, check if phase_vel == dot(group_vel, q)
        self._grad_eig_val = grad_eig
        self._group_velocity = group_vel
        self._cos_pf_angle = dot(self._group_dir, self.direction)
        self._powflow_angle = arccos(around(self._cos_pf_angle, 10))
    ...
    def set_hessian_eig(self):
        ...
        hessian = zeros((3, 3, 3))
        for n in range(3):
            hessian[n] += dot(dot(hess_mat, eig_vec[n]), eig_vec[n])
            # pseudoinv = linalg.pinv(eig_val[n]*idmat - dynmat, rcond=1e-10)
            for i in range(3):
                ...
                if abs(x) < 1e-10:
                    ...
                    diag[i][i] = 1.0 / x
            pseudoinv = dot(dot(eig_vec.T, diag), eig_vec)
            deriv_vec = dot(gradmat, eig_vec[n])
            hessian[n] += 2.0 * dot(dot(deriv_vec, pseudoinv),
                                       deriv_vec.T)  # Take deriv of eigenvec into account: 2 * (d/dx s_i) * pinv_ij * (d_dy s_j)
    ...
    def set_enhancement(self):
        ...
        grad_group = empty((3, 3, 3))
        enhance = empty(3)
        for n in range(3):
            grad_group[n] = hessian[n] / group_abs[n]
            grad_group[n] -= outer(group_vel[n], dot(hessian[n], group_vel[n])) / (group_abs[n] ** 3)
            grad_group[n] /= 2.0 * phase_vel[n]  # grad lambda = 2 * v_p * v_g

            enhance[n] = 1.0 / norm(dot(cofactor(grad_group[n]), self.direction))
    ...
    def set_enhancement_approx(self, num_steps=8, delta=1e-5):
        ...
        """
        phase_grid = empty((num_steps + 1, 3))
        group_grid = empty((num_steps + 1, 3, 3))
        ...
        for i in range(num_steps):
            angle = i * 2.0 * pi / num_steps
            self.set_direction_spherical(center_theta + sin(angle) * delta, center_phi + cos(angle) * delta)
        ...
        # tot_angle = zeros(3)
        for i in range(num_steps):
            phase_area += norm(cross(phase_grid[i] - phase_center, phase_grid[i + 1] - phase_center))
            for n in range(3):
                group_area[n] += norm(
                    cross(group_grid[i][n] - group_center[n], group_grid[i + 1][n] - group_center[n]))
...
def de_voigt(C_ij):
    ...
    C_ijkl = [[[[C_ij[VOIGT[10 * i + j]][VOIGT[10 * k + l]] for i in range(3)] for j in range(3)] for k in range(3)] for
              l in range(3)]
    return C_ijkl
...
def hessian_christoffelmat(C):
    ...
    """
    hessianmat = empty((3, 3, 3, 3))
    for i in range(3):
        for j in range(3):
            for k in range(3):
                for m in range(3):
                    hessianmat[i][j][k][m] = C[k][i][j][m] + C[k][j][i][m]
    return hessianmat
...
def isotropic_velocities(bulk, shear, dens):
    ...
    """
    primary = sqrt(1000.0 * (bulk + 4.0 * shear / 3) / dens)
    secondary = sqrt(1000.0 * shear / dens)
    return primary, secondary


def get_rot_mat(vector1, vector2):
    """Return a rotation matrix that rotates vector2 towards vector1."""
    vector1 = array(vector1) / norm(vector1)
    vector2 = array(vector2) / norm(vector2)
    rotvec = cross(vector2, vector1)
    ...
    ddt = outer(dir_vec, dir_vec)
    skew = array([[0.0, -dir_vec[2], dir_vec[1]], [dir_vec[2], 0.0, -dir_vec[0]], [-dir_vec[1], dir_vec[0], 0.0]])
...
def invert_file(filename, theta_column=None, phi_column=None, cart_columns=[]):
    ...
    for linenumber in range(len(data) - 1, -1, -1):
        ...
        if theta_column is not None:
            line[theta_column] = str(pi - float(line[theta_column]))
...
def determinant(m):
    ...
    return (m[0][0] * m[1][1] * m[2][2] - m[0][0] * m[1][2] * m[2][1] + m[0][1] * m[1][2] * m[2][0] - m[0][1] * m[1][
        0] * m[2][2] + m[0][2] * m[1][0] * m[2][1] - m[0][2] * m[1][1] * m[2][0])
